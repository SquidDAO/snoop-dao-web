...
export const changeApprovalV1 = createAsyncThunk(
  "stake/changeApproval",
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
    ...
    const signer = provider.getSigner();
    const ohmContract = new ethers.Contract(addresses[networkID].OHM_ADDRESS as string, ierc20Abi, signer);
    const sohmContract = new ethers.Contract("0xC96569c5b2c5Ad6115180fa3c97F763Bd13898b3", ierc20Abi, signer);
    let approveTx;
    try {
      if (token === "ohm") {
        approveTx = await ohmContract.approve(
          "0x0d0e8DD8159Ea4b3fB98B9157d9F83aEd339aFdf",
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      } else if (token === "sohm") {
        approveTx = await sohmContract.approve(
          "0xc187197E43e2175247982A7051FdC342936D922E",
          ethers.utils.parseUnits("1000000000", "gwei").toString(),
        );
      }
      const text = "Approve " + (token === "ohm" ? "Staking" : "Unstaking");
    ...
    const stakeAllowance = await ohmContract.allowance(address, "0x0d0e8DD8159Ea4b3fB98B9157d9F83aEd339aFdf");
    const unstakeAllowance = await sohmContract.allowance(address, "0xc187197E43e2175247982A7051FdC342936D922E");
    return dispatch(
      fetchAccountSuccess({
        staking: {
          ohmStake: +stakeAllowance,
...
export const changeStakeV1 = createAsyncThunk(
  "stake/changeStake",
  async ({ action, value, provider, address, networkID }: IActionValueAsyncThunk, { dispatch }) => {
    if (!provider) {
      dispatch(error("Please connect your wallet!"));
    ...
    const signer = provider.getSigner();
    const staking = new ethers.Contract("0xc187197E43e2175247982A7051FdC342936D922E", OlympusStaking, signer);
    const stakingHelper = new ethers.Contract("0x0d0e8DD8159Ea4b3fB98B9157d9F83aEd339aFdf", StakingHelper, signer);
